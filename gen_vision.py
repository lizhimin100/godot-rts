...
def call_api(img, prompt, api_key, model, base_url):
    ...
    payload = {
        ...
        "messages": [{
            "role": "user",
            "content": [
                {"type": "image_url", "image_url": {"url": f"data:{get_mime(img)};base64,{b64}"}},
...
def main():
    ...
    p.add_argument("--provider", "-p", choices=["doubao","qwen","openai"],
                   default=os.environ.get("VISION_PROVIDER", "doubao"))
    p.add_argument("--model", "-m")
    a = p.parse_args()
    if not os.path.exists(a.image):
        print(f"[ERROR] 文件不存在: {a.image}", file=sys.stderr)
        sys.exit(1)
    env_map = {"doubao":"DOUBAO_API_KEY","qwen":"DASHSCOPE_API_KEY","openai":"OPENAI_API_KEY"}
    url_map = {"doubao":"https://ark.cn-beijing.volces.com/api/v3",
    ...
    if not ak:
        print(f"[ERROR] 未设置 {env_map[a.provider]}", file=sys.stderr)
        sys.exit(1)
    model = a.model or os.environ.get("VISION_MODEL", model_map[a.provider])
    base_url = os.environ.get(f"{a.provider.upper()}_BASE_URL", url_map[a.provider])
    print(f"🔍 调用 {a.provider} 视觉模型 ({model})...", file=sys.stderr)
...
# Write to project dir first
with open('E:/类魔兽/类魔兽/vision.py', 'w', encoding='utf-8') as f:
    f.write(code)
print(f"Written {len(code)} bytes to vision.py")

# Write to skills dir
skills_dir = os.path.expanduser('~/.claude/skills/vision')
os.makedirs(skills_dir, exist_ok=True)
target = os.path.join(skills_dir, 'vision.py')
