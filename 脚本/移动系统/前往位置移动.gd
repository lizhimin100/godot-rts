...
func 是否已到达(单位: Node2D, 请求: 移动请求) -> bool:
	...
	_打印到达日志(单位, 请求, 最终目标, false, 距离平方)
	return false


## 到达判定日志
func _打印到达日志(单位: Node2D, 请求: 移动请求, 最终目标: Vector2, 到达: bool, 距离平方: float) -> void:
	if 请求.队形槽位 < 0:
		# 非队形单位不做日志
		return
	var 距离 = sqrt(距离平方)
	var mode = "formation"
	var in_form = 队形系统.实例 and 队形系统.实例.是否在队形中(单位)
	if not in_form:
		mode = "ERROR-no-system"
	print("[ARRIVE-CHECK] <", 单位.name, "> mode=", mode,
		  " arrived=", 到达, " dist=", 距离, " slot_id=", 请求.队形槽位,
		  " target=(", int(最终目标.x), ",", int(最终目标.y), ")")


## 路径方向日志 — 显示实际导航模式和目标
func _打印路径日志(单位: Node2D, 请求: 移动请求, 目标: Vector2, 方向: Vector2, 模式: String = "ff") -> void:
	if 请求.队形槽位 < 0:
		return
	print("[PATH] <", 单位.name, "> mode=", 模式,
		  " target=(", int(目标.x), ",", int(目标.y),
		  ") dir=(", 方向.x, ",", 方向.y, ")")
