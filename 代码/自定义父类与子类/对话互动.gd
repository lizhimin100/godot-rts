...
func 对话交互逻辑(交互者: Node) -> void  :
	...
	#进行对话
	开始对话 = true
	#print("当前阶段:", 当前对话阶段, "/", 对话序列.size())  # DEBUG
	策略相机.聚焦对话位置(global_position)  #  传入NPC的位置，相机锁定
	Dialogic.start(对话序列[当前对话阶段])  #调用插件start方法
	#print("对话实例类型: ", Dialogic.get_class()) # 检查对话实例是否有 timeline_ended 信号  # DEBUG
...
func 角色移动 () :
	if 当前交互者:
		if 当前交互者.has_method("结束对话") :
			当前交互者.结束对话()
			#print("成功调用 当前交互者.结束对话 方法")  # DEBUG
		else:
			#print("当前交互者 节点没有 结束对话 方法")  # DEBUG
	else:
		#print("当前交互者 为空，无法调用 结束对话 方法")  # DEBUG


func _ready() -> void:
	if !策略相机:
		#print("无法找到策略相机节点，请检查节点路径。")  # DEBUG
